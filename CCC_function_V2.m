...
% inside the grids
% load 'C:\Program Files\MATLAB\R2017a\bin\RA_matlab\BP_164dataset.mat'
...
detect_index=1;  % 1 means only detects will be take into the average concentration calculation in girds function
                 % 0 means everything will be take into calculation

temp_test=0;

%%%%%%%%%%% concentrations in differnent groups/clusters %%%%%%
% load 'C:\Program Files\MATLAB\R2017a\bin\beach formation and sea surface pollutant\excel result output\10x10 group setting 2\Settings for clusters.mat'
...
% load 'BP_164dataset.mat'
% for n_name=208:length(File_name)
%     path=['C:\Program Files\ArcGIS\work-RA\RA_paper used data\excel_CCC_V2_unit_united\',...
...
% BPD= BPD(test(:,1),:);
% 
% BP_fulldataset(n_name)={BPD};
% end
%%
for n_name=1:length(File_name)
% load BPD.mat
BPD=BP_252dataset{n_name};
m=n_name;
time=cellstr(BPD(:,1));
Date_s=datenum(time);
...
%%%%%%%%%%%% Sampled date for each group %%%%%%%%%%%%%%%%%%%%%%
Sampled_date(m,group_i)={[unique(x_d)-734248]};
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%             D_C_output(n,2)={BPD(1,5)};
%             D_C_output(n,3)={datestr(T(i))};
%             D_C_output(n,4)={BPD(1,7)};
            if detect_index==1   
                temp_x_c=x_c(I,1);
                D_C(n,2)=nanmean(temp_x_c(temp_x_c~=0)); % only detects
                D_C(n,3)=length(x_c(I,1));  % number of mesurements per date, cluster, chemcial
                D_C(n,4)=nnz(x_c(I,1)>0); % number of detects 
                clear temp_x_c
            else 
...
%
if exist('D_C')
    Days_related_with_Concentrations(m,group_i)={D_C};
    D_C(isnan(D_C))=0;
    [~,C]=size(D_C);
    [i j]=size(Chemical_n_m);
%%%%%%%% format %%%%%%
    Chemical_number_measured(m,1)=File_name(m);
    Chemical_number_measured(m,2)=BPD(1,5);
    Chemical_number_measured(m,3)=BPD(1,7);
    Chemical_number_detected(m,1)=File_name(m);
    Chemical_number_detected(m,2)=BPD(1,5);
    Chemical_number_detected(m,3)=BPD(1,7);
    if i>1
...
%%%%%%%%%%%%%%%% Double Date %%%%%%%%%%%%%%%%%%%%%%%5
    ...
    for j=1:length(I)-1
        if (temp(j+1,2)-temp(j,2))/temp(j,2)>=perc_set
            ...
            temp_Double_Date(temp_n,:)=[temp(j,1),temp(j+1,1)] ;   
...
%             Double_Date(temp_n,i+1)=[];
    ...
    Double_Date(m,group_i)={temp_Double_Date}; % for each chemical, double date was recorded, multiple exposure
    clear temp_Double_Date
...
% clear i I j J k BP
%%
%%%%%%%%%% calculate the HitDate based on 252 chemcials %%%%%%%%%
...
for i=1
    ...
    [h95(i) p95(i) ci95(:,i) stats(group_i)]=ttest(temp_segment,0,'Alpha',0.05);
    end
end
...
%         mark(n_pre,j)=length(HitDate1(~isnan(HitDate1(:,j)),j));
        ...
        error_bar=[nanmean(HitDate1(:,j));nanmean(HitDate2(:,j))];
    else
        error_bar=ci95;
    end
end
...
%     HitDate_bar=NaN;
% end
N_chemcials_doubled_sandy(group_i)=length(find(HitDate1>0));
 HitDate_bar_sandy(group_i)=HitDate_bar
 error_sandy(:,group_i)=error_bar;
else 
    HitDate_bar_sandy(group_i)=NaN;
    error_sandy(:,group_i)=[NaN;NaN];
end
